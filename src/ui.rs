...
fn draw_agents_tab(f: &mut Frame, area: Rect, ctx: &RenderContext) {
    ...
    if ctx.agents.is_empty() {
        ...
        let uninst_p = Paragraph::new(vec![
            Line::from(""),
            Line::from(""),
            Line::from(Span::styled(
                "◌  No agent selected",
        ...
        f.render_widget(uninst_p, chunks[1]);
        return;
    }
    ...
    let meta_rows = vec![
        ...
        Row::new(vec![
            ...
            Cell::new(selected_agent.version.as_deref().unwrap_or("N/A")),
        ]),
        Row::new(vec![
            Cell::new("Config Directory:"),
            Cell::new(selected_agent.config_path.as_deref().unwrap_or("None")),
        ]),
        Row::new(vec![
